    ...
    return (
        <div className={"mx-5"}>
            <div className="flex items-center pb-5">
                ...
                <h1 className="scroll-m-20 text-3xl font-bold tracking-tight lg:text-4xl px-4">
                    {t("pizza")}
                </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {items.map((item, index) => (
                    <CardItems key={index} picture={Picture} title={item.title} description={item.description} price={item.price}/>
                ))}
            </div>
        </div>
    );
