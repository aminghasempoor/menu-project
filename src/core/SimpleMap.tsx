        ...
        <div className="w-full p-4 rounded-xl shadow-md flex flex-col md:flex-row items-center mb-16 gap-6 md:gap-12">
            <div className="w-full md:w-1/2 text-right text-zinc-800 space-y-12">
                <div className="flex items-center gap-2">
                    ...
                    <p className={"flex text-foreground text-md md:text-lg gap-x-2 line-clamp-1"}>{t("address")}{" "}{user?.address}</p>
                </div>
                <div className="flex items-center gap-2">
                    <Phone className="w-6 h-6 text-orange-500" />
                    <p className={"text-foreground text-md md:text-lg"}>{t("number")}{" "}{user?.telephone}</p>
                </div>
            </div>
            <div className="w-full md:w-1/2 h-72 overflow-hidden rounded-lg">
                <MapContainer center={[Number(lat), Number(lng)]} zoom={16} scrollWheelZoom={true}
